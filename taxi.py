...
class Taxi:
      ...
      def __init__(self, world, taxi_num, idle_loss=256, max_wait=50, on_duty_time=0, off_duty_time=0, service_area=None, start_point=None):
          ...
          self.payments = 0
          self.completed_fares = 0
          self.cancelled_fares = 0
      ...
      #Dijkstra Algorithm
      #distance from the origin node to all nodes
         distances = {node: float('inf') for node in self._map.keys()}
         distances[origin] = 0
         # Dictionary for previous nodes
         previous = {}

         queue = [(0, origin)]
         # loop through queue and pop smallest distance
         while queue:
            ...
            if current_node == destination:
               ...
               return list (reversed(path))#return path in the correct order
            # if current distance is greater than the recorded distance
            if current_distance > distances[current_node]:
               continue
            for neighbor, (direction, weight) in self._map[current_node].items():
               distance = current_distance + weight
               #if the path is shorter, update the distance
   ...
   #BFS Search   
#     def pathPlan(self, origin, destination):
...
#             expansion = dict([(neighbor, current_node) for neighbor in self._map[current_node].keys()
...
# Usage example within the class:
#origin = (0, 0)  # Replace with your actual origin
#destination = (5, 5)  # Replace with your actual destination
#path = self.pathPlan(origin, destination)
...
#    print("Path:", path)
 ...
 #   print("No path found.")
      ...
      def _bidOnFare(self, time, origin, destination, price, possible_traffic= False):# If True function will include traffic conditions
          ...
          # calculate the probability. If it the probobility is close enougha and worthwhile then bid.
          BidProbability = 1 if CloseEnough and Worthwhile else 0
          #traffic conditions implemented. 
          #Adjust the probability
          if possible_traffic:
             TrafficProbability = 0.8 if numpy.random.rand()> 0.5 else 1.0
             BidProbability *= TrafficProbability
             #Boolean indicating bid on fare
          return numpy.random.rand() < BidProbability
