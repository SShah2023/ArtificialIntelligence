...
class Dispatcher:
      ...
      def _costFare(self, fare):
          ...
          #use 10 as the base fare
          if timeToDestination < 0:
             return 150
          result =10
          #Traffic factors that will act as traffic factors
          trafficFactor = self._calculateTrafficFactor(fare)
          #See which taxis have fewer than 2 fares
          #See which taxis have fewer than 10 fares
          availableTaxis = len([taxi for taxi in self._taxis if len(taxi._availableFares) <10])
          #Adjust the fare if there are less than 3 taxis, increase the fare by £30 due to demand.
          if availableTaxis < 3:
              result += 30
          demandFactor = self._calculateDemandFactor(fare)
          #Increase the fare based on traffic, location and demand
          result += timeToDestination * trafficFactor
          result *= demandFactor
          return result
      #Generation of traffic factors and demand factors
      def _calculateTrafficFactor( self, fare):
          return 1 + numpy.random.uniform(0, 0.5)
      def _calculateDemandFactor(self, fare):
          return 1 + numpy.random.uniform(0, 0.5)
      ...
      def _allocateFare(self, origin, destination, time):
          # Decide whether the fare is stll relevant
          if self._parent.simTime-time > 3:
             #Initialise shortest time and that no taxis have been allocated
             allocatedTaxi = -1
             shortestTime = float('inf')
             #Get origin fare node 
             fareNode = self._parent.getNode(origin[0],origin[1])
             #Check for active bids
             #Go through all taxis that are a currently bidding on this fare
             #Check if the taxis are valid
             #Check if the taxi currently has passengers and retreive node of the taxis location
             #Calculate total service time if bidder node is valid
             #If total time is less than shortestTime then update the allocation
             if len(self._fareBoard[origin][destination][time].bidders) >= 1:
                             for taxiIdx in self._fareBoard[origin][destination][time].bidders:
                                 if len(self._taxis) > taxiIdx:
                                     taxi = self._taxis[taxiIdx]
                                     if taxi._passenger is None:
                                         bidderNode = self._parent.getNode(taxi.currentLocation[0],taxi.currentLocation[1])
                                         if bidderNode is not None:
                                             totalTimeToService = self._parent.travelTime(bidderNode,fareNode)
                                             if totalTimeToService < shortestTime:
                                                 allocatedTaxi = taxiIdx
                                                 shortestTime = totalTimeToService 
                          
                             # 
                             if allocatedTaxi >= 0:
                                #Update the system about fare allocation
