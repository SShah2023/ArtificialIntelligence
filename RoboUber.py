...
def runRoboUber(worldX,worldY,runTime,stop,junctions=None,streets=None,interpolate=False,outputValues=None,**args):
   ...
   #the main loop that will run for threadRunTime duration
   while threadTime < threadRunTime:
         ...
         if stop.is_set():
            ...
            #This will run simulation for one tick if simulation time is different to threadRunTime, increment threadRunTime.
            if threadTime != svcArea.simTime:
               threadTime += 1
               # short sleep time will change 
            time.sleep(0.01)
   #initialise variable to hold profits
   total_profit = 0
   dispatcher_profit = 0
#calculating total profits of from taxis
   for vehicle in taxis:
       print("Taxi {}: £{:.2f}".format(vehicle.number, vehicle.payments)) #Display taxi earnings
       total_profit += vehicle.payments #Earnings are added to total profits

# calculating the 10% profit for the dispatcher and total profit and printing.
   dispatcher_profit = total_profit * 0.10
   print("Total Profit: £{:.2f}".format(total_profit))
   print("Dispatcher Profit (10%): £{:.2f}".format(dispatcher_profit))

   for vehicle in taxis:
    print(f"Taxi {vehicle.number}: Completed Fares = {vehicle.completed_fares}, Cancelled Fares = {vehicle.cancelled_fares}")
